...
r1=`mincstats -quiet -sum minccalc-out.mnc`
if [ "$r1" != '375' ]; then
...
#
minccalc -clobber -quiet -expression "ratio = A[0]/A[1]; exp(-ratio)" test-one.mnc test-two.mnc minccalc-out.mnc
r1=`mincstats -quiet -sum minccalc-out.mnc`
...
# equal to sqrt(3)*125
#
minccalc -clobber -quiet -expression "x = cos(A[0])+A[1]; sqrt(x)" test-zero.mnc test-two.mnc minccalc-out.mnc
r1=`mincstats -quiet -sum minccalc-out.mnc`
...
#
minccalc -clobber -quiet -expression "s0=s1=s2=0; for {i in [0:len(A))} { v=A[i]; s0 = s0 + 1; s1 = s1 + v; s2 = s2 + v*v; }; stdev = (s0>1) ? sqrt((s2 - s1*s1/s0) / (s0-1)) : (s0 > 0) ? 0 : NaN ; mean = (s0 > 0) ? s1 / s0 : NaN;" test-one.mnc test-one.mnc test-two.mnc -outfile mean minccalc-out.mnc -outfile stdev minccalc-out2.mnc
r1=`mincstats -quiet -sum minccalc-out.mnc`
...
r1=`mincstats -quiet -sum minccalc-out2.mnc`
...
minccalc -clobber -quiet -expression "clamp(A[0]+A[1], 1, 4)" test-one.mnc test-rnd.mnc minccalc-out.mnc
r1=`mincstats -quiet -min minccalc-out.mnc`
if [ "$r1" != '1' ]; then
...
r1=`mincstats -quiet -max minccalc-out.mnc`
if [ "$r1" != '4' ]; then
...
minccalc -clobber -quiet -expression "segment(A[0]*A[1], 1.5, 4.5)" test-two.mnc test-rnd.mnc minccalc-out.mnc
# Since the "random" file contains exactly 25 voxels of value 0,1,2,3, or 4,
...
minccalc -clobber -quiet -expression 'm=0;n=len(A); for {i in (m:n)} { 1; }; m;' test-zero.mnc minccalc-out.mnc
r1=`mincstats -quiet -sum minccalc-out.mnc`
if [ "$r1" != '0' ]; then
...
minccalc -clobber -quiet -expression 'm=0;n=len(A); for {i in (m:n)} { 1; }; n;' test-zero.mnc minccalc-out.mnc
r1=`mincstats -quiet -sum minccalc-out.mnc`
if [ "$r1" != '125' ]; then
